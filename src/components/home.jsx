...
export default function HomePage() {
  ...
  return (
    <div className="bg-gray-100">
      ...
      {/* Hero Section */}
      <div className="relative h-[350px] md:min-h-screen mt-[-110px] flex justify-center items-center text-white text-center overflow-hidden">
        {/* Video Background */}
        <video
          autoPlay
          loop
        ...
        >
          <source src="/images/Sri Lanka _ 4K Cinematic Travel Film(720P_HD)_1.mp4" type="video/mp4" />
        ...
        <div className="relative z-10 bg-black bg-opacity-50 p-6 md:p-10 rounded-lg">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
            <h1 className="text-2xl md:text-6xl font-bold font-serif drop-shadow-lg">
              WELCOME TO SRI LANKA
            </h1>
            <p className="text-lg md:text-2xl mt-4 font-poppins drop-shadow-lg">
              Let’s Celebrate Your Holidays...
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <ScrollAnimation direction="up">
        <div className="text-center py-10 px-4 md:px-0">
          ...
          <div className="mt-4 w-full md:w-4/5 mx-auto">
            <Slider {...settings}>
              {destinations.map((place, index) => (
                <Link key={place.name} to={place.path}>
                  <motion.div
                    whileHover={{ scale: 1.05 }}
                    className="p-4 mb-4 transition-all duration-300 ease-in-out transform"
                  >
